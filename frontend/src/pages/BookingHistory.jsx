...
export default function HistoryInvoice() {
  ...
  useEffect(() => {
    if (user.id != null) fetchInvoiceByUserId(user.id);
  }, [user.id]);
  ...
  const onSubmit = async (data) => {
    if (!selectedFeedback) {
      ...
      const response = await api.put(
        `/review/update/${selectedFeedback.id}`,
        review
      );
      if (response.data.code == 1) {
        toast.success(response.data.message);
  ...
  return (
    <div className="w-[100%] h-auto">
              ...
              >
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-3 justify-center">
                    <button
                      className="text-indigo-600 bg-blue-200 p-2 rounded-md cursor-pointer hover:text-indigo-800 font-medium"
                      onClick={() => {
                        setOpenDetail(true);
                        fetchImgsByHotelId(i.room.hotel.hotelId, i);
                      }}
                    >
                      Xem chi tiết
                    </button>
      ...
      {openReview && (
        ...
        >
          <form
            className="w-[500px] h-auto space-y-5"
                ...
                {iv && (
                  <div className="flex flex-col gap-2 leading-relaxed">
                    ...
                    <p>
                      ...
                      {iv.room.hotel.hotelAddress}
                    </p>
                    <p>
                      <span className="font-medium text-gray-700">Số sao:</span>{" "}
                      {iv.room.hotel.hotelRating} ⭐
                    </p>
                    <p>
                      <span className="font-medium text-gray-700">
                        Liên hệ:
                      </span>{" "}
                      {iv.room.hotel.hotelPhone}
                    </p>
                  </div>
                )}
              </div>
                  ...
                  <div className="flex flex-col gap-2 leading-relaxed">
                    ...
                    <div className="grid grid-cols-2 gap-x-8 gap-y-2 text-sm">
                      ...
                      <p>
                        ...
                        {iv.room.bedRoomCount}
                      </p>
                    </div>

                    {/* --- Phần thanh toán --- */}
                    <div className="border-t border-gray-200 mt-3 pt-2">
                      <h3 className="text-base font-semibold text-amber-600 mb-1 text-center">
                      ...
                      <p>
                        ...
                        <span>{iv.room.roomPrice.toLocaleString()} VNĐ</span>
                      </p>
                      <p>
                        <span className="font-medium text-gray-700">
                          Tổng tiền:
                        </span>
                        <span> {iv.totalAmount.toLocaleString()} VNĐ</span>
                      </p>
                      <p>
                        <span className="font-medium text-gray-700">
                          Thanh toán:
