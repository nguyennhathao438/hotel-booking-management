...
export default function RoomList({ rooms, hotelId }) {
    ...
    return (
        <div className="w-[90%] m-auto">
            {rooms.map((room) => (
                <div key={room.roomId} className="flex flex-col lg:flex-row m-5 p-2 border rounded-2xl shadow-xl bg-gradient-to-r from-blue-50 via-white to-blue-50 hover:from-blue-100 hover:to-white transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl">
                    ...
                    <div className="flex flex-col lg:flex-row w-full lg:w-[70%] p-4 rounded-xl bg-white/70 backdrop-blur-sm">
                        ...
                        <div className="flex justify-center lg:justify-end items-center p-3">
                            <button onClick={() => handleBooking(room.roomId)} className="bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-semibold px-6 py-2 rounded-xl shadow-md hover:from-blue-600 hover:to-indigo-700 hover:shadow-xl hover:scale-105 active:scale-95 transition-all duration-300">
                                Đặt phòng
                            </button>
                        </div>
                    </div>
