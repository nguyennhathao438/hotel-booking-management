...
export default function HotelList() {
  ...
  useEffect(() => {
    const fetchHotelsAndImages = async () => {
      ...
      try {
        ...
        const imagesPromises = hotelsList.map((hotel) => getHotelImages(hotel.hotelId));
  ...
  return (
    <div className="p-1 relative font-serif">
      ...
      <div className="flex flex-row items-center">
        <ul className="flex w-full flex-row gap-3 overflow-x-auto scrollbar-hide md:grid md:grid-cols-1 lg:grid-cols-1 md:gap-6 md:overflow-visible">
          {hotels.map((hotel) => {
            const hotelImages = images.filter((img) => img.hotelId === hotel.hotelId);
            return (
              <li
                key={hotel.hotelId}
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <div className="flex justify-between items-center mt-2">
                    ...
                    <span className="font-semibold text-blue-600">Giá: {hotel.hotelCost.toLocaleString()} VNĐ</span>
                  </div>
                  <p className="mt-1 text-sm">
                    Trạng thái:{" "}
                    <span className={hotel.status === 0 ? "text-yellow-600 font-semibold" : "text-green-600 font-semibold"}>
      ...
      {addRooms && hotels.length > 0 && (
        <div className="mt-6">
          <AddRoom hotelId={hotels[0].hotelId} hotelName={hotels[0].hotelName} />
        </div>
      )}

      {showRooms && hotels.length > 0 && (
        <div className="mt-6">
          <RoomManagerCustomer hotelId={hotels[0].hotelId} hotelName={hotels[0].hotelName} />
        </div>
      )}
      ...
      {DetailHotel && (
        ...
        >
          <div className="border w-[1200px] max-w-full bg-white p-6 rounded-lg shadow-lg relative" onClick={(e) => e.stopPropagation()}>
            <button
            ...
            <EditHotelForm
              ...
              onUpdated={() => {
                api.get(`/hotels/user/${user.userId}`)
