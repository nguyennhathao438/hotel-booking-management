...
const EditHotelForm = ({ onUpdated }) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`https://provinces.open-api.vn/api/p/${provinceCode}?depth=2`, { withCredentials: false })
  ...
  const submitHotelUpdate = async () => {
    ...
    try {
      ...
      if (provinceCode) {
        const province = listProvinces.find((p) => p.code.toString() === provinceCode);
  ...
  return (
    <div className="w-[1700px] items-center justify-center min-h-screen bg-gray-100 ml-[200px]">

      <div className="w-[1550px] flex bg-white shadow-lg rounded-2xl gap-20 p-8">
        {/* Form thông tin khách sạn */}
        <div className="w-[800px]">
          <h1 className="text-2xl font-semibold text-center mb-6">Quản lý khách sạn</h1>
          ...
          <div>
            <label className="block font-semibold mb-1">Điện thoại liên hệ</label>
            <input
              name="hotelPhone"
